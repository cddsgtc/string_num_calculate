...
void main() {
  ...
  // 减法计算
  test('static [substract]计算加', () {
    expect(Calculate.substract(1, 2), -1);
    expect(Calculate.substract(1, -2), 3);
  });
  // 乘法计算
  test('static [multiply]计算加', () {
    expect(Calculate.multiply(1, 2), 2);
    expect(Calculate.multiply(1, -2), -2);
    expect(Calculate.multiply(1, 0), 0);
  });
  // 除法计算
  test('static [divide]计算加', () {
    expect(Calculate.divide(1, 2), .5);
    expect(Calculate.divide(1, -2), -.5);
  });
  // 小费计算
  test('static [percent]计算加', () {
  ...
  test('Calculate->[LinierCalculate]计算一个字符串计算', () {
    expect(Calculate.LinierCalculate('1+3+5'), 9);
    expect(Calculate.LinierCalculate('-1+3+5'), 7);
